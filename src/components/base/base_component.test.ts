...
describe('Component', () => {
  ...
  it('should require a render function', () => {
    expect(() => { Component({} as any) }).toThrow('A render() function is required')
  })

  it('should not complain about render function if it\'s provided', () => {
    expect(() => { Component({'render': () => 0} as any) })
  ...
  const loader_test = (args: any) => {
    ...
    const my_loader = sinon.spy(function(url: any, callback: any) {
  ...
  it('loader is called with render args - file loader 1', () => {
    ...
    const my_file_loader = (path: any, callback: any) => {
  ...
  it('re-subscribes to state changes', () => {
    ...
    const state_handler = {
      'get_state': sinon.spy(),
      'subscribe': sinon.spy(function(f: any) {callback = f})}
    call_test_component_with({
      'instance_args': {
        'state_handler': state_handler
      },
