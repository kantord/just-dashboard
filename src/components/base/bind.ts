...
const create_element = (
  init_func: ComponentConfig['init'],
  instance_args: ComponentArgs,
  selection: D3Selection,
): D3Selection => {
  try {
    return typeof init_func === 'function'
      ? (init_func(instance_args, selection) as D3Selection)
...
const create_bind_function = (args: ComponentConfig, instance_args: ComponentArgs) => (selection: D3Selection) => {
  ...
  let element: D3Selection | null = create_element(args.init, format_arguments(instance_args), selection)
  ...
  return (raw_data: unknown) => {
    ...
    if (has_state_handler(instance_args)) {
      instance_args.state_handler!.subscribe((state_handler, me) => {
        if (element === null || (element?.node && !document.contains(element.node() as Node))) return
