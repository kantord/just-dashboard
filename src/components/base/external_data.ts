...
const load_external_data =
  (raw_data: string) =>
  (
    loader_func: (source: string) => (callback: (...args: unknown[]) => void) => void,
    spinner_func: (
...
const has_loader = (instance_args: ComponentArgs | undefined): boolean =>
  instance_args !== undefined && Object.hasOwn(instance_args, 'loader')

const handle_external_data =
  (instance_args: ComponentArgs, selection: D3Selection, raw_data: unknown) => (resolve: (data: unknown) => void) =>
    has_loader(instance_args)
      ? load_external_data(raw_data as string)(
          with_loader(
            instance_args.loader as string,
            instance_args.file_loader as any,
