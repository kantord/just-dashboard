...
const format_arguments = (args: ComponentArgs): ComponentArgs => {
  if (!has_state_handler(args)) return args
  return format_value(args, args.state_handler!.get_state()) as ComponentArgs
}

const format_data = (args: ComponentArgs, data: unknown): unknown => {
  if (!has_state_handler(args)) return data
  return format_value(data, args.state_handler!.get_state())
}

const has_state_handler = (args: ComponentArgs | undefined): boolean => {
