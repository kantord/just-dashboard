...
type TestSuite = [string, unknown[][][]]

const tests: TestSuite[] = [
  ['String interpolation', [string_tests]],
  ['Array interpolation', [array_tests]],
  ['Value interpolation', [string_tests, array_tests, static_tests, object_tests]],
  ['Object interpolation', [object_tests]],
]

const stringify = JSON.stringify

tests.forEach(([test_suite_name, test_sets]) =>
    ...
    ;(test_sets as unknown[][][]).forEach((test_set: unknown[][]) =>
      test_set.forEach(([input, output, state]) =>
        it(`${[stringify(input), stringify(output), stringify(state)]}`, () => {
          expect(format_value(input, state as Record<string, unknown>)).toEqual(output)
        }),
      ),
    )
  }),
