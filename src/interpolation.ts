...
const format_string = (input: string, state: State): string => {
  ...
  return format_string(input.replace(`\${${first_key}}`, String(state[first_key])), copy)
}
...
const format_object = (input: Record<string, unknown>, state: State): Record<string, unknown> => {
  return Object.assign(
    {},
    ...Object.entries(input).map(([key, value]) => ({ [format_string(key, state)]: format_value(value, state) })),
  )
}
